...
database = os.getenv("AZURE_SQL_DATABASE")  # Default to AdventureWorks2022 if not set
...
def run_sql_query_enhanced(args: EnhancedSqlQueryArgs) -> list:
    """
    Enhanced SQL query for Azure SQL database (sales schema) with exploration tracking and automatic insights generation.
    """
    ...
    try:
        if conn is None:
            return [TextContent(type="text", text="Azure SQL connection is not initialized.")]
...
# Tool: List all Azure SQL table names and their columns/attributes
...
def list_db_tables(args: ListTablesArgs = None) -> list:
    """
    List all table names in the connected Azure SQL database and their columns/attributes (sales schema).
    """
    global conn
    try:
        if conn is None:
            return [TextContent(type="text", text="Azure SQL connection is not initialized.")]
        # List all tables and views in the 'Sales' schema only (case-sensitive)
        query_tables = """
            SELECT TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE
            FROM INFORMATION_SCHEMA.TABLES
            WHERE TABLE_SCHEMA = 'Sales' AND TABLE_TYPE IN ('BASE TABLE', 'VIEW')
            ORDER BY TABLE_NAME
        """
        tables_df = pd.read_sql(query_tables, conn)
        result = "=== DATABASE TABLES AND VIEWS (Sales schema only) ===\n\n"
        for _, row in tables_df.iterrows():
            table = row['TABLE_NAME']
            ttype = row['TABLE_TYPE']
            # Get columns for each table/view in Sales schema
            query_columns = f"""
                SELECT COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, IS_NULLABLE
                FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_SCHEMA = 'Sales' AND TABLE_NAME = '{table}'
            """
            columns_df = pd.read_sql(query_columns, conn)
            col_str = ", ".join([
                f"{r['COLUMN_NAME']} ({r['DATA_TYPE']}{'[' + str(r['CHARACTER_MAXIMUM_LENGTH']) + ']' if r['CHARACTER_MAXIMUM_LENGTH'] else ''}, nullable={r['IS_NULLABLE']})"
                for _, r in columns_df.iterrows()
            ])
            result += f"{ttype}: {table}\n  Columns: {col_str}\n\n"
        result += "\nTip: Use the correct object name (e.g., Sales.vSalesPersonSalesByFiscalYears) in your queries."
        return [TextContent(type="text", text=result)]
    except Exception as e:
...
def notes_resource():
    ...
    global _notes
    return "\n".join(_notes) if _notes else "No notes yet. Use run_sql_query_enhanced or run_script to generate notes."

if __name__ == "__main__":
    # --- Connect to Azure SQL and share the connection globally ---
    try:
        print("[Startup] Initializing Azure SQL connection...")
        init_azure_sql_connection()
        print("[Startup] Azure SQL connection initialized and shared globally.")
    except Exception as e:
        conn = None
        print(f"[Startup] Azure SQL connection initialization failed: {e}")
